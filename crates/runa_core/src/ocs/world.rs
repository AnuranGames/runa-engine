...
use crate::{
    ...
    components::{
        AudioListener, AudioSource, Mesh, MeshRenderer, SpriteRenderer, Tilemap, Transform,
    },
...
impl World {
    ...
    pub fn render(&self, render_queue: &mut RenderQueue, interpolation_factor: f32) {
        for object in &self.objects {
            // 3D Mesh rendering
            if let (Some(transform), Some(mesh), Some(mesh_renderer)) = (
                object.get_component::<Transform>(),
                object.get_component::<Mesh>(),
